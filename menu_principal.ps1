...
# --- CONFIGURAR AMBIENTE E DOWNLOADS ---
...
$csvPath   = Join-Path $caminhoTemp "dados_impressoras.csv"
$motorPath = Join-Path $caminhoTemp "instalar_universal.ps1"
        ...
        do {
            ...
            elseif ($modo -eq "2") {
                ...
                do {
                    ...
                    } else { Write-Host "2 - Easy Printer Manager (EPM)" }
                    Write-Host "v - Voltar"
                    
                    $peca = Read-Host "`nO que deseja instalar?"
                    if ($peca -eq "v") { $sairPersonalizado = $true; break }

                    $params.instalarPrint = $false; $params.instalarScan = $false; $params.instalarEDC = $false; $params.instalarEPM = $false
                    if ($dados.TemScan -eq "S") {
                        switch ($peca) { "1" {$params.instalarPrint=$true} "2" {$params.instalarScan=$true} "3" {$params.instalarEDC=$true} "4" {$params.instalarEPM=$true} }
                    } else {
                        switch ($peca) { "1" {$params.instalarPrint=$true} "2" {$params.instalarEPM=$true} }
                    }
                    & $motorPath @params
